...
import com.jvmbytes.spy.event.ImmediatelyReturnEvent;
import com.jvmbytes.spy.event.ImmediatelyThrowsEvent;
...
public class EventListenerHandlers {
    ...
    private Spy.Ret handleEvent(final int listenerId,
        ...
        try {
            ...
            if (contains(processor.eventTypes, event.eventType)) {
                // 调用事件处理
        ...
        // 代码执行流程变更
            ...
            final State state = pce.getState();
            ...
            switch (state) {
                ...
                case RETURN_IMMEDIATELY: {
                    ...
                    if (!contains(processor.eventTypes, EventType.IMMEDIATELY_RETURN)) {
                ...
                case THROWS_IMMEDIATELY: {
                    ...
                    // 如果没有注册监听ImmediatelyEvent事件，则不进行后续的操作
                    if (!contains(processor.eventTypes, EventType.IMMEDIATELY_THROWS)) {
